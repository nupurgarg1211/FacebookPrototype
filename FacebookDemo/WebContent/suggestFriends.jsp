							...
							<div class="row">
								...
								<div class="col-sm-8">
									<div class="well">
										...
										<h4 align="left">Suggest Friends</h4>
										<hr />
 
										<%
											ArrayList<UserDhruvik> userData=(ArrayList<UserDhruvik>)request.getAttribute("usersuggestData");
			 ...
			 %>  --%>
										...
										<div class="row">
											<s:form class="form-horizontal" id="dhruvik"
												action="suggestFriend">
												<table>

													<%
														for(int j=0;j<userData.size();j=j+1){
															UserDhruvik u11=(UserDhruvik)userData.get(j);
													%>
													<%
														if (j%2==0) {
													%>
													<tr>
														...
														<td>
														<input type="hidden" id="temp11" name="suggestid" value="<%=u11.getUserid()%>">
															<ul class="list-group">
																<li class="list-group-item" style="width: 300px"><img
																	src="<%=u11.getProfile_pic()%>" class="img-rounded"
																	width="100" height="80" />&nbsp;<a
																	href="userNewsFeedAction?user_id=<%=u11.getUserid()%>"
																	style="color: #3B5999"><b><%=u11.getFirst_name()%>
																			<%=u11.getLast_name()%></b></a><br />
																	
														 	<%-- 		 <%if(show_jsp==1){ %> --%>
																	<div align="right">
																		<input type="button" id="<%=u11.getUserid()%>"
																			value="Suggest Friend" 
																			onclick="hello('<%=u11.getUserid()%>')">
																	</div>
															<%-- 	<%} %> --%>
																	</li>
												...
												<input type="hidden" id="temp1" name="user_id" value="<%=tempUserId%>">
											</s:form>
